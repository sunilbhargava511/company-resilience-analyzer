...
export const formatResult = (text) => {
  ...
  // Remove standalone "#" symbols (markdown remnants)
  html = html.replace(/^\s*#\s*$/gm, '');
  
  // Remove redundant parenthetical section labels like "(Strengths)", "(Risks)", "(Opportunities)"
  html = html.replace(/^\s*\([A-Za-z\s&]+\)\s*$/gm, '');
  
  // Remove duplicate "Opportunities" text that appears separately from main headers
  html = html.replace(/^\s*Opportunities\s*$/gm, '');
  ...
  html = html.replace(/(#{1,3}\s*📊\s*Company Overview[\s\S]*?)(?=#{1,3}\s*[🔋⚠️🎯🚀📈💡🔮]|$)/gi, (match) => {
    ...
    if (Object.keys(companyData).length > 0) {
      ...
      let overviewHTML = `
        <div class="my-4 p-4 bg-white rounded-lg shadow-md border border-slate-300">
  ...
  html = html.replace(/#{1,3}\s*🔋\s*Resilience Drivers/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-emerald-700 dark:text-emerald-400 flex items-center gap-3 border-b border-emerald-200 dark:border-emerald-700 pb-2">
      🔋 Resilience Drivers
    </h2>`);
    
  html = html.replace(/#{1,3}\s*⚠️\s*Vulnerability Factors/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-red-700 dark:text-red-400 flex items-center gap-3 border-b border-red-200 dark:border-red-700 pb-2">
      ⚠️ Vulnerability Factors
    </h2>`);
    
  html = html.replace(/#{1,3}\s*🎯\s*Competitive Landscape/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-purple-700 dark:text-purple-400 flex items-center gap-3 border-b border-purple-200 dark:border-purple-700 pb-2">
      🎯 Competitive Landscape
    </h2>`);
    
  html = html.replace(/#{1,3}\s*🚀\s*Adjacent Market/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-blue-700 dark:text-blue-400 flex items-center gap-3 border-b border-blue-200 dark:border-blue-700 pb-2">
      🚀 Adjacent Market Opportunities
    </h2>`);
    
  html = html.replace(/#{1,3}\s*📈\s*Key Performance/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-indigo-700 dark:text-indigo-400 flex items-center gap-3 border-b border-indigo-200 dark:border-indigo-700 pb-2">
      📈 Key Performance Metrics
    </h2>`);

  html = html.replace(/#{1,3}\s*🎯\s*Bottom Line/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-yellow-700 dark:text-yellow-400 flex items-center gap-3 border-b border-yellow-200 dark:border-yellow-700 pb-2">
      🎯 Bottom Line
    </h2>`);
    
  html = html.replace(/#{1,3}\s*💡\s*Portfolio Positioning/gi, 
    `<h2 class="text-2xl font-bold mt-6 mb-4 text-cyan-700 dark:text-cyan-400 flex items-center gap-3 border-b border-cyan-200 dark:border-cyan-700 pb-2">
  ...
  html = html.replace(/#{4}\s*([^#\n]+)/g, 
    `<h4 class="text-base font-semibold mt-3 mb-2 text-gray-800 dark:text-gray-200">$1</h4>`);
  ...
  html = html.replace(/^[-•]\s+(.+)$/gm, '<li class="mb-1 text-gray-700 dark:text-gray-300">$1</li>');
  
  // Group consecutive list items (remove list-disc to avoid double bullets)
  html = html.replace(/(<li[^>]*>.*?<\/li>[\s\n]*)+/gs, (match) => {
    return `<ul class="ml-6 mb-4 space-y-1">${match}</ul>`;
  });
  ...
  html = html.split('\n\n').map(paragraph => {
    ...
    if (paragraph && 
      ...
      if (/^(Strengths?|Risks?|Opportunities|Metrics?\s*[&]?\s*Benchmarks?|Scoring\s+Breakdown)[\s:]*$/i.test(paragraph)) {
        return `<h3 class="text-lg font-bold mt-4 mb-2 text-gray-800 dark:text-gray-200">${paragraph}</h3>`;
      }
      ...
      return `<p class="mb-3 text-gray-700 dark:text-gray-300 leading-relaxed">${paragraph}</p>`;
    }
    return paragraph;
  }).join('\n\n');
  ...
  return `<div class="prose prose-base max-w-none text-gray-900 dark:text-gray-100" style="line-height: 1.4;">${scoreDisplay + html}</div>`;
};