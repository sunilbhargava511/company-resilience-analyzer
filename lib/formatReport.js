...
export const formatResult = (text) => {
  ...
  html = html.replace(/(#{1,3}\s*📊\s*Company Overview[\s\S]*?)(?=#{1,3}\s*[🔋⚠️🎯🚀📈💡🔮]|$)/gi, (match) => {
    ...
    if (Object.keys(companyData).length > 0) {
      ...
      let overviewHTML = `
        <div class="my-6 p-3 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg shadow-sm border border-slate-200">
        ...
        if (companyData[key]) {
          overviewHTML += `
            <div class="flex flex-col">
              <span class="font-semibold text-slate-700 mb-1">${key}:</span>
              <span class="text-slate-600 leading-relaxed">${companyData[key]}</span>
            </div>
          `;
        }
      });
