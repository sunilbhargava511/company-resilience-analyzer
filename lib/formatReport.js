...
export const formatResult = (text) => {
  ...
  html = html.split('\n\n').map(paragraph => {
    ...
    if (paragraph && 
      ...
      // Special handling for important conclusion text (like Bottom Line summary)
      if (paragraph.length > 50 && /demonstrates|expansion|competitive|pressure|defensibility|resilience/i.test(paragraph)) {
        return `<div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 rounded-r-lg">
          <p class="text-gray-800 dark:text-gray-200 leading-relaxed font-medium">${paragraph}</p>
        </div>`;
      }
